...
from core import MusicDownloader, BatchParser, SongMatcher
...
class BatchSearchWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            from core import BatchSongMatch, BatchSearchResult, MatchCandidate
            import time
...
class ConcurrentSearchWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            from core import BatchSearchResult
            result = BatchSearchResult(
                total_songs=total_songs,
                matches=matches,
                search_time=search_time,
    ...
    def run(self):
        ...
        try:
            ...
            for i, song in enumerate(self.songs):
                ...
                # 连接信号 - ⚠️ 修复：收集完整的song_dict而不是只有歌曲名
                # 使用默认参数song_capture=song来避免闭包陷阱
                runnable.signals.success.connect(
                    lambda name, song_capture=song: (
                        # 使用捕获的song_capture而不是song
                        successful_songs.append(song_capture),
            ...
            # 计算
            success_count = len(successful_songs)
            success_rate = success_count / len(self.songs) if self.songs else 0

            logger.info(
                f"[ConcurrentDownload] Completed: {success_count} successful, "
            ...
            # ⚠️ 修复：发送完整的song_dict列表而不是歌曲名字符串列表

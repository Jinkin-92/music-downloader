...
class MatchCandidate:
    ...
    # 相似度分解字段（用于前端展示详细信息）
    name_similarity: float = 0.0  # 歌名相似度
    singer_similarity: float = 0.0  # 歌手相似度
    album_similarity: float = 0.0  # 专辑相似度

    # 下载相关字段
    download_url: Optional[str] = None  # 直接下载URL
    duration_s: int = 0  # 时长（秒），用于过滤
    song_id: Optional[str] = None  # 缓存ID，用于从服务器获取SongInfo对象
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "similarity": self.similarity_score,  # 兼容前端
            "name_similarity": self.name_similarity,
            "singer_similarity": self.singer_similarity,
            "album_similarity": self.album_similarity,
            # 下载相关字段
